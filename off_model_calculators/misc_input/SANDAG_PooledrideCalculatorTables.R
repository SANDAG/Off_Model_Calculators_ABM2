...
Scenario=c("2020","2025nb","2035nb","2035_D","2035_Fx","2035_E_minus_test2")
for (s in Scenario){

# ----------------------------------------------------------------------
# set working directory, data directory and input file names here
main_dir    <- paste0("T:/RTP/2019RP/rp19_scen/abm_runs_bod/",s,"/")
dataDir     <- paste0(main_dir,"r scripts/")  # where input files are
...
trip_summary <- indiv_trip_temp %>%
  mutate( mode=ifelse(between(trip_mode,1,2),"DA",ifelse(between(trip_mode,3,8),"SR",ifelse(between(trip_mode,9,10),"NM","TRN"))),
...
skim_summary <- skim_summary_temp %>%
  ...
  summarise(avg_dattime=mean(Travel_time_DA), avg_srtime=mean(Travel_time_SR), avg_distance=mean(Distance)) %>%
  mutate(year=YEAR)


# write summary tables out!    
write.csv(trip_summary, paste0(output_dir,"pooledride_trips",YEAR,".csv"), row.names = F)
write.csv(skim_summary, paste0(output_dir,"pooledride_times",YEAR,".csv"), row.names = F)
}