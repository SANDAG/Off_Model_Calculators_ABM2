...
found in the "off_model_calculators" folder. There are 7 subfolders.
...
* R_scripts - contains all the source codes which are needed to generate inputs
materials for calculator. The application of each script is explained in the 
...
* Model Data NEV Shuttle (by year, orig_msa, dest_msa, orig_nevshuttle_flag, dest_nevshuttle_flag)       
	* person_trips
	* auto_trips    	
	These are generated by "SANDAG_MicrotransitCalculatorTables.R" script. The output of script is named "Model Data NEV Shuttle_/scenario name/_Date.csv". Once the script run is finished
    this file is accessible from "R_script/output". The table then shall be pasted into Model Data NEV Shuttle tab	
* Model Data CB Shuttle (by orig_msa, dest_msa, year) 
	...
	* Average Trip Distance, Full Fare 		
	Source of this tab is SummaryTables tab in ABMTransitModeShare.xlsx
    For creating ABMTransitModeShare.xlsx ,"SANDAG_TAP_TAP_to_MAZ_MAZ_IVT_OVT.R" needs to be run first. The script outputs a file "HtWTrips_/Scneario name/_/Date/.csv" in the "R_script/output" 
    which should be pasted to the HtWtips tab in ABMTransitModeShare.xlsx  	
...
* Model Trip Data [pooledride_trips_2035_E_minus_bu_2018-09-24.csv]
	* Trips (by year, origin msa, destination msa, period, mode, purpose, number of autos) : 
	"SANDAG_PooledrideCalculatorTables.R" script outputs "pooledride_trips_/Scneario name/_/Date/.csv" in the "R_script/output" . This file needs to be pasted to the Model Trip Data tab
* Model Skims Data (by year, origin msa, destination msa, period) [pooledride_times_2035_E_minus_bu_2018-09-24.csv]
	* avg_datime	
	* avg_srtime	
	* avg_distance
	"SANDAG_PooledrideCalculatorTables.R" script outputs "pooledride_times_/Scneario name/_/Date/.csv" in the "R_script/output" . This file needs to be pasted to the Model Skims Data tab
* Emission Factors
	* Confirm use of 2014 EMFAC or 2017 EMFAC values from ABM 14.0.0
	* Updated EMFAC values
...
* Vanpool Demand FOR ML (Non-Mil), ML (mil)
	...
	* Average travel time savings (by origin msa, destination msa, by year)
	"SANDAG_TravelTimeSaving.R" and "SANDAG_TravelTime2016.R" scripts output "ttsaving_/Scneario name/_/Date/.csv" and "ttsaving_/Scneario name/_/Date/.csv" in the "R_script/output" 
	These files should be pasted to the corresponding tables for miliatry and non miliatry tabs
